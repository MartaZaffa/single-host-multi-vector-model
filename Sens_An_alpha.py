...
interference curvature parameter /alpha on the result.
...
# Declare the parameter that is variated and define its variation
# range

test_param = ['k'] # declare the name of the varied parameter
var_range = [1e3,700e3] # set the variation range limits
step = 5000
k_ave = params['k']

alpha_values = [0.5,1.0,5.0] 
x_lim = ([80e3,400e3],[30e3,150e3],[5e3,30e3]) # limits for the x axis in the plot 
                                                 # (each plot corresponds to an interference curvature scenario)
sz_alpha = len(alpha_values)

# calculation of the value of the interference strength parameter for the direct
# and indirect interference scenarios

nu_dir = params["nu"]
nu_ind = params["nu"]*k_ave**(-1)
   
if len(test_param) < 2:
    test_values = np.linspace(var_range[0], var_range[1], step)
       
    R0 = np.zeros((len(test_values), sz_alpha))
    R0R = np.zeros((len(test_values), sz_alpha))
    R0T = np.zeros((len(test_values), sz_alpha))
    i_eq = np.zeros((len(test_values), sz_alpha))
    Nc_eq = np.zeros((len(test_values), sz_alpha))
    
    R0_2 = np.zeros((len(test_values), sz_alpha))
    R0R_2 = np.zeros((len(test_values), sz_alpha))
    R0T_2 = np.zeros((len(test_values), sz_alpha))
    i_eq_2 = np.zeros((len(test_values), sz_alpha))
    Nc_eq_2 = np.zeros((len(test_values), sz_alpha))
    
    # sensitivity analysis I_eq
